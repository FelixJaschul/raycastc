...
static int load_sectors(const char *path) {
    ...
    // print to confirm file is opened
    ...
    while (fgets(line, sizeof(line), f)) {
        ...
        if (!*p || *p == '#') {
            ...
            case SCAN_WALL: {
                ...
                if (sscanf(p, "%d %d %d %d %d", &wall->a.x, &wall->a.y, &wall->b.x, &wall->b.y, &wall->portal) != 5)
                    retval = -4; goto done;
            }
            case SCAN_SECTOR: {
                struct sector *sector = &state.sectors.arr[state.sectors.n++];
                if (sscanf(p, "%d %zu %zu %f %f", &sector->id, &sector->firstwall, &sector->nwalls, &sector->zfloor, &sector->zceil) != 5)
                    retval = -5; goto done;
            }
...
static bool point_in_sector(const struct sector *sector, v2 p) {
    for (usize i = 0; i < sector->nwalls; i++) {
        ...
        if (point_side(p, v2i_to_v2(wall->a), v2i_to_v2(wall->b)) > 0) return false;
    }
...
static void render() {
    ...
    while (queue.n != 0) {
        ...
        for (usize i = 0; i < sector->nwalls; i++) {
            ...
            const int wallshade = 16 * (sin(atan2f( wall->b.x - wall->a.x, wall->b.y - wall->b.y)) + 1.0f);
            ...
            const f32
                ...
                z_ceil = sector->zceil,
                nz_floor = wall->portal ? state.sectors.arr[wall->portal].zfloor : 0,
                nz_ceil = wall->portal ? state.sectors.arr[wall->portal].zceil : 0;
            ...
            for (int x = x0; x <= x1; x++) {
                ...
                if (wall->portal) {
                    ...
                    state.y_hi[x] =
                        clamp(min(min(yc, nyc), state.y_hi[x]), 0, SCREEN_HEIGHT - 1);

                    state.y_lo[x] =
                        clamp(max(max(yf, nyf), state.y_lo[x]), 0, SCREEN_HEIGHT - 1);
                }
                else verline(x, yf, yc, abgr_mul(0xFFD0D0D0, shade));
            ...
            if (wall->portal)
                queue.arr[queue.n++] = (struct queue_entry) { .id = wall->portal, .x0 = x0, .x1 = x1 };
        }
    }
...
int main(int argc, char *argv[]) {
    state.window =
        SDL_CreateWindow("raycast", SDL_WINDOWPOS_CENTERED_DISPLAY(0), SDL_WINDOWPOS_CENTERED_DISPLAY(0), 1280, 720, 0);
    state.renderer =
        SDL_CreateRenderer(state.window, -1, SDL_RENDERER_ACCELERATED | SDL_RENDERER_PRESENTVSYNC);
    state.texture =
        SDL_CreateTexture(state.renderer, SDL_PIXELFORMAT_ABGR8888, SDL_TEXTUREACCESS_STREAMING, SCREEN_WIDTH, SCREEN_HEIGHT);
    state.debug =
        SDL_CreateTexture(state.renderer, SDL_PIXELFORMAT_ABGR8888, SDL_TEXTUREACCESS_TARGET, 128, 128);
    state.pixels =
        malloc(SCREEN_WIDTH * SCREEN_HEIGHT * 4);
