...
static void render_g() {
    ...
    const v2 zdl = rotate(((v2) { 0.0f, 1.0f }), +(HFOV / 2.0f));
    const v2 zdr = rotate(((v2) { 0.0f, 1.0f }), -(HFOV / 2.0f));
    const v2 znl = (v2) { zdl.x * ZNEAR, zdl.y * ZNEAR };
    const v2 znr = (v2) { zdr.x * ZNEAR, zdr.y * ZNEAR };
    const v2 zfl = (v2) { zdl.x * ZFAR, zdl.y * ZFAR };
    const v2 zfr = (v2) { zdr.x * ZFAR, zdr.y * ZFAR };
    ...
    while (queue.n != 0) {
        ...
        for (usize i = 0; i < sector->nwalls; i++) {
            ...
            const v2 op0 = world_pos_to_camera(v2i_to_v2(wall->a));
            const v2 op1 = world_pos_to_camera(v2i_to_v2(wall->b));
            ...
            if (cp0.y < ZNEAR
                ...
                const v2 ir = intersect_segs(cp0, cp1, znr, zfr);
            ...
            const int wallshade = 16 * \
					(sin(atan2f(wall->b.x - wall->a.x, wall->b.y - wall->b.y)) + 1.0f);

            const int x0 = clamp(tx0, entry.x0, entry.x1);
            const int x1 = clamp(tx1, entry.x0, entry.x1);

            const f32 z_floor  = sector->zfloor;
            const f32 z_ceil   = sector->zceil;
            const f32 nz_floor = wall->portal ? state.sectors.arr[wall->portal].zfloor : 0;
            const f32 nz_ceil  = wall->portal ? state.sectors.arr[wall->portal].zceil : 0;
            ...
            const int nyfd = nyf1 - nyf0;
            const int nycd = nyc1 - nyc0;
            ...
            for (int x = x0; x <= x1; x++) {
                ...
                const int tyf = (int) (xp * yfd) + yf0;
                const int tyc = (int) (xp * ycd) + yc0;
                const int yf  = clamp(tyf, state.y_lo[x], state.y_hi[x]);
                const int yc  = clamp(tyc, state.y_lo[x], state.y_hi[x]);
                ...
                if (wall->portal) {
                    const int tnyf = (int) (xp * nyfd) + nyf0;
                    const int tnyc = (int) (xp * nycd) + nyc0;
                    const int nyf  = clamp(tnyf, state.y_lo[x], state.y_hi[x]);
                    const int nyc  = clamp(tnyc, state.y_lo[x], state.y_hi[x]);

                    verline(x, nyc, yc, abgr_mul(0xFF00FF00, shade));

                    verline(x, yf, nyf, abgr_mul(0xFF0000FF, shade));

                    state.y_hi[x] = clamp(min(min(yc, nyc), state.y_hi[x]), 0, SCREEN_HEIGHT - 1);

                    state.y_lo[x] = clamp(max(max(yf, nyf), state.y_lo[x]), 0, SCREEN_HEIGHT - 1);
                } else {
...
static void present() {
    ...
    SDL_LockTexture(state.texture, NULL, &px, &pitch);
    {
        for (usize y = 0; y < SCREEN_HEIGHT; y++) {
            memcpy(&((u8*) px)[y * pitch], &state.pixels[y * SCREEN_WIDTH], SCREEN_WIDTH * 4);
        }
    }
...
int main() {
    ...
    state.window = SDL_CreateWindow("raycast", SDL_WINDOWPOS_CENTERED_DISPLAY(0),
			SDL_WINDOWPOS_CENTERED_DISPLAY(0), 1280, 720, 0);
