...
void render_game(void) {
    // Debugging: Render-Funktion aufgerufen
    printf("render_game() wird ausgeführt...\n");
    ...
    for (i32 x = 0; x < SCREEN_WIDTH; x++) {
        ...
        // Debugging: Überprüfen, ob Strahlen korrekt berechnet werden
        printf("Strahl für Spalte %d berechnet.\n", x);
    ...
    // Debugging: Render-Funktion abgeschlossen
    printf("render_game() abgeschlossen.\n");
}
...
int main(void) {
    ...
    // Speicher für state.pixels reservieren
    state.pixels = (u32*)malloc(SCREEN_WIDTH * SCREEN_HEIGHT * sizeof(u32));
    if (!state.pixels) {
        fprintf(stderr, "Fehler: Speicher für state.pixels konnte nicht reserviert werden.\n");
        return 1;
    }
    ...
    // Debugging: Überprüfen, ob die Initialisierung korrekt ist
    printf("Initialisierung abgeschlossen. Start der Hauptschleife...\n");
    ...
    while (!state.quit) {
            ...
            switch (ev.type) {
                ...
                case SDL_MOUSEBUTTONDOWN:
                    if (state.mode == 1 && ev.button.button == SDL_BUTTON_LEFT) {
                        ...
                        for (int i = 0; i < state.wall_count; i++) {
                            const v2i pts[2] = {state.walls[i].p0, state.walls[i].p1};
                            for (int j = 0; j < 2; j++) {
                                const int vx = pts[j].x;
                                const int vy = pts[j].y;
                                const f32 d = hypotf((f32)(mx - vx), (f32)(my - vy));
        ...
        // Debugging: Überprüfen, ob der Render-Modus korrekt ist
        printf("Aktueller Modus: %d\n", state.mode);
        ...
        if (state.mode == 0) {
            ...
            // Debugging: Überprüfen, ob state.pixels korrekt ist
            if (!state.pixels) {
                fprintf(stderr, "Fehler: state.pixels ist NULL.\n");
                break;
            }
            ...
            // Debugging: Render abgeschlossen
    ...
    // Speicher für state.pixels freigeben
    free(state.pixels);
